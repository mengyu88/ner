...
class Conv2d_selfAdapt(nn.Module):
    def __init__(
        ...
        self.softArgMax = Soft_argmax(t=self.t, topk=2)
    ...
    def _local_sparse_attention_response(self, x):
        ...
        logits = (q.unsqueeze(2) * k_unfold).sum(dim=1) / (hidden_size ** 0.5 + 1e-12)
        ...
        topk = min(max(self.attn_topk, 1), logits.size(1))
        sparse = soft_topk_mask(logits, topk=topk, temperature=self.t)
        temperature = max(float(self.t), 1e-6)
        dense = F.softmax(logits / temperature, dim=1)
    ...
    def forward(self, x, init_flag):
        ...
        kernel_size = self.kernel_size
        if self.local_sparse_attn:
            return self._local_sparse_attention_response(x)
        x_unfold = self.unfold(x).reshape(batch_size, hidden_size, kernel_size * kernel_size, h, w)
        if init_flag:
            mask_x = self.mask_conv2d(x)
            if self.relation_bias:
                rel_feat = self._build_relation_feat(h, w, x.device, mask_x.dtype)
                rel_bias = self.rel_bias_mlp(rel_feat)
        ...
        x_unfold[:, :, [0, 1, 2, 3, 5, 6, 7, 8], :, :] = (
            x_unfold[:, :, [0, 1, 2, 3, 5, 6, 7, 8], :, :] * mask
        )
        if self.dynamic_depthwise:
