...
def build_test_loader(data_bundle, matrix_segs, batch_size):
    ...
    return prepare_torch_dataloader(
        test_ds,
        batch_size=batch_size,
        # Avoid tokenizer-fork deadlock during standalone evaluation.
        num_workers=0,
...
def collect(metrics_map, thresholds):
    rows = []
    for t in thresholds:
        m = metrics_map[t].get_metric()
...
def main():
    ...
    parser.add_argument("--thresholds", type=str, default="0.48:0.56:0.01")
    parser.add_argument(
        "--alphas",
        type=str,
        default="0.2:0.8:0.05",
        help="weight for model1 in ensemble: ensemble=alpha*m1+(1-alpha)*m2",
    )
    ...
    ensemble_map = {
        a: {t: NERMetric(matrix_segs=matrix_segs, ent_thres=t, allow_nested=True) for t in thresholds}
        for a in alphas
    }
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for a in alphas:
                out_e = a * out1 + (1.0 - a) * out2
                for t in thresholds:
                    ensemble_map[a][t].update(ent_target=batch["ent_target"], scores=out_e, word_len=batch["word_len"])

    rows1 = collect(single1, thresholds)
    rows2 = collect(single2, thresholds)
    best1 = max(rows1, key=lambda x: x["f1"])
    best2 = max(rows2, key=lambda x: x["f1"])

    ensemble_summary = []
    for a in alphas:
        rows = collect(ensemble_map[a], thresholds)
        best = max(rows, key=lambda x: x["f1"])
        ensemble_summary.append({"alpha": a, "best": best, "rows": rows})

    best_ensemble = max(
        [{"alpha": item["alpha"], **item["best"]} for item in ensemble_summary],
        key=lambda x: x["f1"],
    )
    ...
    payload = {
        ...
        "ensemble_by_alpha": [
            {"alpha": item["alpha"], "best": item["best"]} for item in ensemble_summary
        ],
