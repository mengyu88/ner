...
def convert_split(input_path, output_path, split_name):
    ...
    with output_path.open("w", encoding="utf-8") as w:
        for idx, (tokens, tags) in enumerate(iter_sentences(input_path)):
            ...
            sample = {
                "tokens": tokens,
                "doc_id": f"conll03-{split_name}-{idx}",
                "sent_id": f"conll03-{split_name}-{idx}",
...
def main():
    ...
    parser.add_argument(
        "-i",
        "--input_dir",
        default=str(Path(__file__).resolve().parents[1] / "data" / "Conll03"),
        help="Directory containing train.txt/valid.txt/test.txt",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        default=str(Path(__file__).resolve().parent / "outputs" / "conll03"),
